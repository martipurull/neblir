...
export function ItemDetailModal({
  ...
  const weaponDamage =
    isWeapon && item && "damage" in item ? item.damage : null;
  const baseDamageDice = weaponDamage?.numberOfDice ?? 0;
  const baseDamageType = weaponDamage?.diceType ?? 0;
  const totalDamageDice = Math.max(0, baseDamageDice + damageExtraDice);

  const handleRollDamage = () => {
    if (!weaponDamage || totalDamageDice <= 0 || baseDamageType <= 0) return;
  ...
  return (
    <>
              ...
              {removeConfirmOpen ? (
                <div className={innerActionPanelClass}>
                  ...
                  <p className="text-sm text-white/85">
                    Remove{" "}
                    <span className="font-medium text-white">{name}</span> from
                    this character? This cannot be undone.
                  </p>
                  <DangerButton
                    type="button"
                    onClick={() => {
      ...
      {damageRollOpen && weaponDamage && (
        <div
          className="fixed inset-0 z-[70] flex items-center justify-center bg-black/50 px-4"
          role="dialog"
          aria-modal="true"
          aria-labelledby="item-damage-roll-title"
          onClick={() => setDamageRollOpen(false)}
        >
          <div
            ...
            <div className="mt-3 space-y-3 text-sm">
              ...
              <div className="flex items-center justify-between gap-4">
                ...
                <div className="flex items-center gap-2">
                  <button
                    type="button"
                    onClick={() => setDamageExtraDice((v) => v - 1)}
                ...
                <div className="rounded border border-white/20 bg-black/20 p-2">
                  ...
                  <p className="text-sm text-white">
                    ...
                    <span className="font-bold">
                      {damageRollResult.reduce((sum, v) => sum + v, 0)}
                    </span>
                  </p>
                </div>
              )}
              <p className="text-xs text-white/60">
                {weaponDamage.damageType?.join(", ") || "Damage"}.
              </p>
              <button
                type="button"
                onClick={() => setDamageRollOpen(false)}

...
export const PATCH = auth(async (
    ...
    try {
        if (!request.auth?.user) {
            logger.error({ method: 'PATCH', route: '/api/characters/[id]/notes', message: 'Unauthorised access attempt' })
        ...
        if (!id || typeof id !== 'string') {
            logger.error({ method: 'PATCH', route: '/api/characters/[id]/notes', message: 'Invalid character ID', characterId: id })
        ...
        if (!characterBelongsToUser(request.auth?.user?.characters, id)) {
            logger.error({ method: 'PATCH', route: '/api/characters/[id]/notes', message: 'Character does not belong to user', characterId: id })
        ...
        if (error) {
            logger.error({ method: 'PATCH', route: '/api/characters/[id]/notes', message: 'Error parsing notes update request', details: error })
        ...
        logger.error({ method: 'PATCH', route: '/api/characters/[id]/notes', message: 'Error updating notes', error })

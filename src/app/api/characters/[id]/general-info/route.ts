...
export const PATCH = auth(async (
    ...
    try {
        if (!request.auth?.user) {
            logger.error({ method: 'PATCH', route: '/api/characters/[id]/general-info', message: 'Unauthorised access attempt' })
        ...
        if (!id || typeof id !== 'string') {
            logger.error({ method: 'PATCH', route: '/api/characters/[id]/general-info', message: 'Invalid character ID', characterId: id })
        ...
        if (!characterBelongsToUser(request.auth?.user?.characters, id)) {
            logger.error({ method: 'PATCH', route: '/api/characters/[id]/general-info', message: 'Character does not belong to user', characterId: id })
        ...
        if (error) {
            logger.error({ method: 'PATCH', route: '/api/characters/[id]/general-info', message: 'Error parsing general information update request', details: error })
            return NextResponse.json({ message: error.issues }, { status: 400 })
        }
        const existingCharacter = await getCharacter(id)
        if (!existingCharacter) {
            logger.error({ method: 'PATCH', route: '/api/characters/[id]/general-info', message: 'Character not found', characterId: id })
        ...
        logger.error({ method: 'PATCH', route: '/api/characters/[id]/general-info', message: 'Error updating general information', error })

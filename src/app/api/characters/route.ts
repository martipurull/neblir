...
export const POST = auth(async (request: AuthNextRequest) => {
    ...
    try {
        if (!user || !user.id) {
            logger.error({ method: 'POST', route: '/api/characters', message: 'Unauthorised access attempt' })
            return errorResponse('Unauthorised', 401)
        }

        const dbUser = await getUser(user.id)
        if (!dbUser) {
            logger.error({ method: 'POST', route: '/api/characters', message: 'No user found in DB' })
        ...
        if (!parseResult.success) {
            logger.error({ method: 'POST', route: '/api/characters', message: 'Error parsing character creation request', details: parseResult.error.issues });
        ...
        try {
            ...
            if (error instanceof ValidationError) {
                logger.error({ method: 'POST', route: '/api/characters', message: 'Error while computing character attributes or skills.', details: error });
                return errorResponse('Error while computing character attributes or skills.', 400, JSON.stringify(error))
            } else {
                logger.error({ method: 'POST', route: '/api/characters', message: 'Error while computing character creation data', details: error });
        ...
        try {
            ...
            logger.error({ method: 'POST', route: '/api/characters', message: 'Error while adding character to user', details: error });
        ...
        logger.error({ method: 'POST', route: '/api/characters', message: 'characters route POST error', details: error })

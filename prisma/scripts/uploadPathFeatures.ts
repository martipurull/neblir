...
async function main() {
    const featureCsvPath = '/Users/martipurull/Documents/Neblir/Game/data/Feature_Upload.csv';
    const pathCsvPath = '/Users/martipurull/Documents/Neblir/Game/data/Path_Upload.csv';
    ...
    // --- Parse Features ---
    const featuresCsv: FeatureCsv[] = parse(fs.readFileSync(featureCsvPath).toString(), {
        columns: true,
    ...
    const features: FeatureDoc[] = featuresCsv.map((f) => ({
        ...
        applicablePaths: f.applicablePaths ? f.applicablePaths.split(';').map(x => x.trim()) : [],
    }));
    ...
    features.forEach((feature, idxFeature) => {
        ...
        feature.applicablePaths.forEach((pathName) => {
            ...
            if (!pathId) {
                console.warn(`Path "${pathName}" not found for feature "${feature.name}"`);
                return;
            }
    ...
    if (pathFeatureDocs.length > 0) {
        ...
        console.log(`Inserted ${pathFeatureDocs.length} PathFeature join documents.`);
    } else {

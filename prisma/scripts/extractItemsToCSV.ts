...
function parseAttackRoll(text: string, itemName: string = ""): string[] {
  ...
  if (
    ...
    upper.includes("RANGED") ||
    upper.includes("RANGED ATTACK")
  )
    types.push("RANGE");
  if (upper.includes("MELEE") || upper.includes("MELEE ATTACK"))
    types.push("MELEE");
  if (upper.includes("GRID") || upper.includes("GRID ATTACK"))
    types.push("GRID");
  if (
    (throwWordRegex.test(upper) || upper.includes("THROWN")) &&
    !hasThrowInName
  )
    types.push("THROW");
  ...
  if (types.length === 0) {
    if (
      ...
      upper.includes("BLADE") ||
      upper.includes("CLUB") ||
      upper.includes("HAMMER")
    ) {
      types.push("MELEE");
...
function parseDamage(text: string): {
  ...
  if (radiusMatch) {
    ...
    const secondaryMatch = text.match(
      /(?:half|secondary|up to|up to)\s*(?:damage|distance)?\s*(?:up to|at)?\s*(\d+)\s*m/i
    );
    if (secondaryMatch) {
...
function determineType(
  ...
  const hasAttackRoll =
    sections.has("attack roll") || sections.has("attackroll");
...
function parseItem(result: GoogleDocResult): ParsedItem {
  ...
  item.description = sections.get("description") || sections.get("desc") || ""; // If no explicit description, might need manual review
  ...
  if (item.type === "WEAPON") {
    const attackRollText =
      sections.get("attack roll") || sections.get("attackroll") || "";
    const attackRollTypes = parseAttackRoll(attackRollText, itemName);
    item.attackRoll =
      attackRollTypes.length > 0 ? JSON.stringify(attackRollTypes) : "";

    const damageText = sections.get("damage") || "";
    const damage = parseDamage(damageText);
    if (damage.numberOfDice)
      item.damageNumberOfDice = damage.numberOfDice.toString();
    if (damage.diceType) item.damageDiceType = damage.diceType.toString();
    if (damage.damageType) item.damageType = damage.damageType;
    if (damage.primaryRadius)
      item.damagePrimaryRadius = damage.primaryRadius.toString();
    if (damage.secondaryRadius)
      item.damageSecondaryRadius = damage.secondaryRadius.toString();
